...
function createFallbackDownload(defaultFormat, defaultLabel, options = {}) {
  const resolveFormat = typeof options.resolveFormat === "function"
    ? options.resolveFormat
    : (_url, requestedFormat) => requestedFormat || defaultFormat;
  const resolveLabel = typeof options.resolveLabel === "function"
    ? options.resolveLabel
    : (_format) => defaultLabel;

  return async function fallbackDownload(url, requestedFormat) {
    const format = resolveFormat(url, requestedFormat);
